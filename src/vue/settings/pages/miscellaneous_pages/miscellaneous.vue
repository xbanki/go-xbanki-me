<template>
    ...
    <main class="component-miscellaneous">
        ...
        <section class="miscellaneous-bar">
            ...
            <span v-bind:class="{ interactable: state.active != 'MISC_TAB_DEFAULT' }" class="bar-title" v-on:click="handle_return_click"> Miscellaneous </span>

            <!-- Bar slash separator, that is only visible outside of the default view //-->
            <span class="bar-separator" v-if="state.active != 'MISC_TAB_DEFAULT'"> / </span>

            <!-- Sub-route (current page) label //-->
            <span class="bar-label" v-if="state.active != 'MISC_TAB_DEFAULT'"> {{ state.label }} </span>
        </section>
